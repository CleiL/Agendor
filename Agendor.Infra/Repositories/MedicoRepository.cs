...
namespace Agendor.Infra.Repositories
{
       ...
       : BaseRepository(uow), IMedicoRepository
    {
        ...
        public async Task<Medico> CreateAsync(Medico entity, CancellationToken cancellationToken = default)
        {
            ...
            {
                const string sql = """
                    insert into Medicos (MedicoId, Nome, CRM, Email, Especialidade)
                    values (@MedicoId, @Nome, @CRM, @Email, @Especialidade)
                    """;
        ...
        {
            const string sql = @"select MedicoId from Medicos where lower(Email) = lower(@email) limit 1";
